...
def render_all_desktop_graphs(wind_tab, wave_tab, tide_tab, precip_tab, hour_data):
    ...
    # ※もし属性名が異なる場合は合わせます
    precips = [getattr(v, 'precipitation_probability', 0) for v in filtered_items.values()]
    ...
    def draw_precip_temp(parent, hours, precips, temps):
        ...
        bars = ax1.bar(hours, precips, color=color_precip, alpha=0.6, width=0.6, label="降水確率")
        ax1.tick_params(axis='y', labelcolor=color_precip)
        ax1.set_ylim(0, 100)
        ax1.set_xlim(SafetyRule.ACTIVITY_START_HOUR - 0.5, SafetyRule.ACTIVITY_END_HOUR + 0.5)
        ax1.set_xticks(range(SafetyRule.ACTIVITY_START_HOUR, SafetyRule.ACTIVITY_END_HOUR + 1))
        
        # 右軸：気温（折れ線グラフ）
        ax2 = ax1.twinx()
        color_temp = '#ff4500' # オレンジレッド
        ax2.set_ylabel("気温 (℃)", color=color_temp)
        line = ax2.plot(hours, temps, color=color_temp, marker="s", linewidth=2, label="気温")
